...
def main():
    ...
    for num, scale in enumerate(range(1, 100+1), start=0):
        ...
        ### SAVING OUT THE DATA ###
        summary_file = os.path.join(s.data_folder, 'stars_lam_-100', 'generated_stars.csv')

        with open(summary_file, 'a+') as f:
            #converting to cgms units
            f.write(f'{rho_c},{Tc},{R_star},{M_star},{L_star},{T_star},{num}\n')

        df.to_csv(rf"{os.path.join(s.data_folder, 'stars_lam_-100', f'star_{num}.csv')}", index=False)
